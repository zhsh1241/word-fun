...
function WordLibrary({ onBack }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 当前主题信息 */}
      <div className={`bg-gradient-to-r ${wordThemes[theme].color} rounded-2xl p-4 text-white border-2 border-white/20`}>
        <div className="flex items-center justify-between">
          <div>
            <div className="text-2xl font-bold mb-1">
              {wordThemes[theme].emoji} {wordThemes[theme].name}
            </div>
            <div className="text-white/90">
              共 {wordThemes[theme].words.length} 个单词
            </div>
          </div>
          <div className="text-5xl">{wordThemes[theme].emoji}</div>
        </div>
      </div>
